...
INSERT INTO game_config (key, value, description) VALUES
  ...
  ('bonus_multiplier',      '1',    'Current global coin multiplier (1 = normal, 2 = double event)'),
  ('registration_required', 'false','Force registration before first play'),
  ('otp_provider',          'msg91','OTP provider: msg91 | twilio')
ON CONFLICT (key) DO NOTHING;

-- ── Leaderboard daily VIEW ─────────────────────────────────────────────────
-- Used by the leaderboard page — aggregates best score per user per day.
...
-- ── RPC: credit_coins ─────────────────────────────────────────────────────
-- Credits YD Coins to a user's wallet, capped by the daily_coin_cap config.
...
RETURNS void LANGUAGE plpgsql AS $$
BEGIN
  UPDATE users SET total_sessions = total_sessions + 1, last_active = NOW()
  WHERE id = p_user_id;
END;
$$;
...
-- Game sessions: insert own, read own
...
-- Leaderboard view is public (no PII — only display_name and score)
-- game_config is readable by all authenticated users (read-only from client)
